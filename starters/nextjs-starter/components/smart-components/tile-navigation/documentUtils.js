 ...
 * 
 * @param {any} documentIds - The document IDs in various formats
 * @returns {string[]} Array of clean document IDs
 */
export function getDocIDsFromProps(documentIds) {
  ...
  try {
    ...
    const docIds = docLinksOrIDs
      .map((linkOrDocID) => {
        try {
          // Handle Google Doc links with more precise regex
