...
export const persistGoogleAuthDetails = async (
  ...
  const existingAccounts = await getGoogleAuthDetails();
  const newAccounts = [];
  if (existingAccounts) newAccounts.push(...existingAccounts);
  newAccounts.push(payload);
  await persistDetailsToFile(newAccounts, GOOGLE_AUTH_FILE_PATH);
};
